...
export class DatosService {
  constructor(@InjectModel('Datos') private datosModel: Model<DatosType>) {}
   ...
   * @returns
   */
  async insertDatos(datosInstance: CrearDatosDTO): Promise<Datos> {
    const {
      id,
      indicador,
      ambito,
  ...
  async updateDatos(datos: CrearDatosDTO) {
    return await this.datosModel.updateOne(
      { id: datos.id },
      {
        $set: {
