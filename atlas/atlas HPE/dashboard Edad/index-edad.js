...
function getDataIndicadores() {
  return $.ajax({
    dataType: "json",
    type: "GET",
    url: "http://localhost:8080/datos/indicadores/333" //Se corresponde a la url del widget. Le falta el id del documento en concreto el cual será el id del dashboard
  })
}
...
myViewModel.globalIndicador.subscribe(function () {
  ...
  if (myViewModel.globalAggLevel() && myViewModel.globalIndicador()) {
    ...
    myViewModel.setVariabilidad('Edad_65_79')

    Promise.all([getDataEdad_65_79(indicador), getLeyendaEdad_65_79(indicador)]).then(values => {

      const datos = values[0].filter(dato => dato.distintivo == 'Edad_65_79')[0]
      let valores = [datos, values[1]]

      const datosParse = parseador(escala, datos.data)
      myViewModel.setData('Edad_65_79', datosParse)

      getMapaEdad_65_79(valores)
    })
...
function getLeyendaEdad_65_79(indicador) {
  return $.ajax({
    dataType: 'json',
    type: 'GET',
...
//JavaScript perteneciente al widget de mapa/gráfico
function getDataEdad_65_79(indicador) {
  return $.ajax({
    dataType: 'json',
    type: 'GET',
    //La url del widget de mapa
...
const getMapaEdad_65_79 = function (datos) {
  const { id, data, nivel } = datos[0];
  const { customLabels, customBreaks } = datos[1];

  let mapa = mapSpain('#mapEdad_65_79'); // --> se pasa como argumento el id del mapa {{{id_mapa}}}
...
myViewModel.globalIndicador.subscribe(function () {
  ...
  if (myViewModel.globalAggLevel() && myViewModel.globalIndicador()) {
    ...
    let escala = myViewModel.globalAggLevel();
    let indicador = myViewModel.globalIndicador();
    myViewModel.setVariabilidad('Edad_80')

    Promise.all([getDataEdad_80(indicador), getLeyendaEdad_80(indicador)]).then(values => {

      const datos = values[0].filter(dato => dato.distintivo == 'Edad_80')[0]
      let valores = [datos, values[1]]

      const datosParse = parseador(escala, datos.data)
      myViewModel.setData('Edad_80', datosParse)

      getMapaEdad_80(valores)
    })
...
function getLeyendaEdad_80(indicador) {
  return $.ajax({
    dataType: 'json',
    type: 'GET',
    //La url del widget de mapa
...
function getDataEdad_80(indicador) {
  return $.ajax({
    dataType: 'json',
    type: 'GET',
    //La url del widget de mapa
...
const getMapaEdad_80 = function (datos) {

  const { id, data, nivel } = datos[0];
  const { customLabels, customBreaks } = datos[1];

  let mapa = mapSpain('#mapEdad_80'); // --> se pasa como argumento el id del mapa {{{id_mapa}}}
